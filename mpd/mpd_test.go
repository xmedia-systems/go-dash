...
func TestNewMPDLiveWithBaseURLInMPD(t *testing.T) {
	m := NewMPD(DASH_PROFILE_LIVE, VALID_MEDIA_PRESENTATION_DURATION, VALID_MIN_BUFFER_TIME)
	m.BaseURL = &BaseURL{Value: new(string)}
	*m.BaseURL.Value = VALID_BASE_URL_VIDEO
	require.NotNil(t, m)
	expectedMPD := &MPD{
		XMLNs:                     Strptr("urn:mpeg:dash:schema:mpd:2011"),
		Profiles:                  Strptr((string)(DASH_PROFILE_LIVE)),
	...
	expectedMPD.BaseURL.Value = new(string)
	*expectedMPD.BaseURL.Value = VALID_BASE_URL_VIDEO
